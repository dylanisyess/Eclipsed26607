...
public class Swerve_drive extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            boolean rightStickDeadZone = false;
            boolean leftStickDeadZone = false;
            double leftPower;
            double rightPower;
            double radius;
            radius = 1;
...
//            rightPower = Math.sqrt(Math.pow(gamepad1.right_stick_y, 2) + Math.pow(gamepad1.right_stick_x, 2));
            ...
            // Manually added deadzones
            // Left Joystick
            if ((leftStickX < deadzone & leftStickX > -1 * deadzone) & (leftStickY < deadzone & leftStickY > -1 * deadzone)) {
                leftStickDeadZone = true;
            }
            // Right Joystick
            if ((rightStickX < deadzone & rightStickX > -1 * deadzone) & (rightStickY < deadzone & rightStickY > -1 * deadzone)) {
                rightStickDeadZone = true;
            }
            ...
            //LeftJoystick Movement (Translational)
            if ((left_theta > servoPos - joystick_variability & left_theta < servoPos + joystick_variability) || (first_translational && (leftStickDeadZone))) {
                if (left_theta < 0) {
                    left_forward = -1.0;
            ...
            // RightJoystick Movement (Rotational)
            if (!rightStickDeadZone) {


                // Quadrant 1
                if (rightStickX < 0 & rightStickY <= 0) {
                    rightwheel.setPower((-1 + 2 * rightStickY) * right_magnitude * magnitude_gain);
                    leftwheel.setPower(right_magnitude * magnitude_gain);
                }
                // Quadrant 2
                if (rightStickX >= 0 & rightStickY < 0) {
                    rightwheel.setPower(right_magnitude * magnitude_gain);
                    leftwheel.setPower((-1 + 2 * rightStickY) * right_magnitude * magnitude_gain);
                }
                // Quadrant 3
                if (rightStickX > 0 & rightStickY >= 0) {
                    rightwheel.setPower((1 + 2 * rightStickY) * right_magnitude * magnitude_gain);
                    leftwheel.setPower(-1 * right_magnitude * magnitude_gain);
                }
                // Quadrant 4
                if (rightStickX <= 0 & rightStickY > 0) {
                    rightwheel.setPower(-1 * right_magnitude * magnitude_gain);
                    leftwheel.setPower((1 + 2 * rightStickY) * right_magnitude * magnitude_gain);
                }
            }
            // independent
